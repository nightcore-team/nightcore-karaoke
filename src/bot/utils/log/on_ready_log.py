...
def walk_group(
    ...
    for cmd in group.commands:
        if isinstance(cmd, app_commands.Group):
            yield (f"{prefix}{cmd.name}", cmd)
            # рекурсія
            yield from walk_group(cmd, prefix=f"{prefix}{cmd.name} ")
        else:
            yield (f"{prefix}{cmd.name}", cmd)
...
def summarize_tree(tree: app_commands.CommandTree) -> Result:
    ...
    for top in top_cmds:
        if isinstance(top, app_commands.Group):
            ...
            leaf_count = 0
            for qname, obj in walk_group(top, prefix=f"{top.name} "):
            ...
            result["top_level"].append(
                {"name": top.name, "type": "leaf", "qualified": top.name}
            )
            overall_leaf += 1
